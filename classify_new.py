...
def train_val_split(imagePath, ratio):
    ...
    train_voc_list = [i.replace('.png', '.txt').replace('.jpg', '.txt').replace('images','label')
             for i in train_pic_list] # 將.png和.jpg都替換為.txt並將images改為label(把原先images資料夾改為label)
    val_pic_list = imagePath[train_num:]
    val_voc_list = [i.replace('.png', '.txt').replace('.jpg', '.txt').replace('images','label')
             for i in val_pic_list]
...
if __name__ == '__main__':
    ...
    train_pic_list,train_voc_list, val_pic_list,val_voc_list = train_val_split(imagePath, 0.8)
